...
<div></div>

      {
       searchResult.map((s)=>(
            ...
            s.type==="song" && 
            <div className='col-md-3 col-sm-11 col-11 text-center mt-4 card-style-search' key={s.position}>
                   <Link to={`details/${s.song.id}`} aria-label="Close" className='text-decoration-none text-dark'>

            <div className="card w-20 card-img-top card-img-top-search">
  <img className='card-img-top card-img-top-search' src={s.song.album.image.cover.url} alt={s.song.album.name.replace('Single' , '')}/>
